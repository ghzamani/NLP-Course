...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    softmax_matrix = softmax(np.dot(outsideVectors, centerWordVec))
    loss = -np.log(softmax_matrix[outsideWordIdx])

    gradCenterVec = -outsideVectors[outsideWordIdx] + np.dot(outsideVectors.T, softmax_matrix)

    # case w=o: -v_c + v_c.y_hat
    # case w!=o: v_c.y_hat
    gradOutsideVecs = np.dot(np.expand_dims(softmax_matrix, axis=1), np.expand_dims(centerWordVec, axis=1).T)
...
def negSamplingLossAndGradient(
    ...
    sigmoid_uo_v = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    sigmoid_uk_v = sigmoid(np.dot(-outsideVectors[negSampleWordIndices], np.expand_dims(centerWordVec, axis=1)))
    loss = -np.log(sigmoid_uo_v) - np.sum(np.log(sigmoid_uk_v), axis=0)

    gradCenterVec = - outsideVectors[outsideWordIdx] * (1-sigmoid_uo_v) + np.sum(outsideVectors[negSampleWordIndices] * (1-sigmoid_uk_v), axis=0)

    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = -(1 - sigmoid_uo_v) * centerWordVec
    for i, neg_idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_idx] += (1 - sigmoid_uk_v[i]) * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for out in outsideWords:
        out_idx = word2Ind[out]
        out_loss, grad_center, grad_out = word2vecLossAndGradient(center_vec, out_idx, outsideVectors, dataset)
        loss += out_loss
        gradCenterVecs[center_idx] += grad_center
