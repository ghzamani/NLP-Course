...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            # adding a first word -> second word to the dependency list
            w = self.stack.pop(-2)
            self.dependencies.append((self.stack[-1], w))

        elif transition == 'RA':
            # removes the first item from the stack, 
            # adding a second word -> first word dependency to the dependency list
            w = self.stack.pop(-1)
            self.dependencies.append((self.stack[-1], w))
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses):
        ...
        for item in minibatch:
            if len(item.buffer) == 0 and len(item.stack) == 1:
