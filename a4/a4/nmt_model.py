...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens = torch.swapaxes(enc_hiddens, 0, 1)
        # step 3
        init_decoder_hidden = torch.concat((last_hidden[0], last_hidden[1]), dim=1)
        init_decoder_hidden = self.h_projection(init_decoder_hidden)

        init_decoder_cell = torch.concat((last_cell[0], last_cell[1]), dim=1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, dim=2)) # shape: (b, src_len, 1)
        ...
        # step 2
        a_t = torch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens) # shape: (b, 1, 2h)
        a_t = torch.squeeze(a_t, dim=1) # shape: (b, 2h)
        # step 3
        U_t = torch.concat((dec_hidden, a_t), dim=1)
        # step 4
