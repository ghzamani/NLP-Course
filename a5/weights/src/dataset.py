...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # step 2
        masked_len = random.randint(0, int(len(document)/2))         
        start_idx = random.randint(1, length-masked_len-1)
        ...
        # step 3
        masked_without_pad = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        pad_len = self.block_size - len(masked_without_pad)
        masked_string =  masked_without_pad + self.PAD_CHAR*pad_len
